...
def test_address_benchmark_route_passes_filters_to_read_only_service(monkeypatch):
    ...
    assert payload["filters"] == {"pickup": "Fort Worth", "delivery": "Dallas"}
    assert client.dataset_calls == [
        {"pickup": "Fort Worth", "delivery": "Dallas", "days": 180}
    ]
...
def test_address_benchmark_route_trims_blank_filters(monkeypatch):
    ...
    assert response.status_code == 200
    assert response.json()["filters"] == {"pickup": None, "delivery": "Dallas"}
    assert client.dataset_calls == [
        {"pickup": None, "delivery": "Dallas", "days": None}
    ]
