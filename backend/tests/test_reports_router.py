...
import _cache as cache_module  # noqa: E402
...
def test_report_generation_returns_stale_source_cache_when_refresh_fails(monkeypatch):
    clear_cached_prefix("report:")
    monkeypatch.setenv("FLEETPULSE_REPORT_CACHE_SECONDS", "60")
    monkeypatch.setenv("FLEETPULSE_REPORT_STALE_CACHE_SECONDS", "120")
    fake_client = FakeGeotabClient()
    monkeypatch.setattr(reports.GeotabClient, "get", lambda: fake_client)
    monkeypatch.setattr(
        reports,
    ...
    original = reports._generate_report_payload("weekly")
    cache_module._cache["report:weekly"] = (time.time() - 61, original)
    monkeypatch.setattr(
        reports.GeotabClient,
        "get",
        lambda: (_ for _ in ()).throw(RuntimeError("OverLimitException API calls quota exceeded")),
    )

    stale = reports._generate_report_payload("weekly")

    assert stale["source_status"] == "stale_source_cache"
    assert stale["summary"] == original["summary"]
    assert stale["source_warnings"] == ["Current Geotab report refresh failed: RuntimeError"]
