...
def get_address_benchmark_dataset(
    *,
    ...
    period_end = _date_in_timezone(now, effective_config.timezone)
...
def build_address_benchmark_dataset(
    ...
    *,
    ...
    config = config or AddressBenchmarkConfig.from_env()
    now = _ensure_aware(now or datetime.now(timezone.utc))
    period_end = _date_in_timezone(now, config.timezone)
...
def _normalize_route_instance(
    ...
    *,
    ...
    route_date = (
        ...
        or (_date_in_timezone(start_at, config.timezone) if start_at else None)
        or (_date_in_timezone(finish_at, config.timezone) if finish_at else None)
    )
    if not route_date:
        return None
...
def _first_date(
    ...
    *,
    ...
    if parsed:
        return _date_in_timezone(parsed, timezone_name)
    return None
...
def _configured_timezone(timezone_name: str):
    try:
...
def _date_in_timezone(value: datetime, timezone_name: str) -> date:
    return _ensure_aware(value).astimezone(_configured_timezone(timezone_name)).date()
