...
HR_WORKBOOK_PATH="${HR_RECRUITING_WORKBOOK_PATH:-/home/data/HR_Lead_KPI_Recheck_By_Phone.xlsx}"
HR_CONVERSION_WORKBOOK_PATH="${HR_RECRUITING_CONVERSION_WORKBOOK_PATH:-/home/data/HR_Lead_Name_To_Xcelerator_Driver_Conversion.xlsx}"
HR_SOURCE="${HR_RECRUITING_SOURCE:-hr_kpi_workbook}"
# A configured workbook path is the deployed-safe route. The snapshot import is
# legacy fallback only when workbook mode is intentionally left unconfigured.
if [[ -n "$HR_WORKBOOK_PATH" ]]; then
  HR_SOURCE="hr_kpi_workbook"
fi
HR_CALL_ANALYSIS_STATE_PATH="${HR_CALL_ANALYSIS_STATE_PATH:-/home/data/fleetpulse_hr_call_analysis.json}"
DEPARTMENT_CALL_ANALYSIS_STATE_PATH="${DEPARTMENT_CALL_ANALYSIS_STATE_PATH:-$HR_CALL_ANALYSIS_STATE_PATH}"
...
GRAPH_TENANT_ID_SECRET_NAME="${FLEETPULSE_GRAPH_TENANT_ID_SECRET_NAME:-}"
GRAPH_CLIENT_ID_SECRET_NAME="${FLEETPULSE_GRAPH_CLIENT_ID_SECRET_NAME:-}"
GRAPH_CLIENT_SECRET_SECRET_NAME="${FLEETPULSE_GRAPH_CLIENT_SECRET_SECRET_NAME:-}"

GRAPH_APP_SETTINGS=()
if [[ -n "$GRAPH_TENANT_ID_SECRET_NAME" ]]; then
  GRAPH_APP_SETTINGS+=(
    "FLEETPULSE_GRAPH_TENANT_ID=@Microsoft.KeyVault(SecretUri=https://${KV_NAME}.vault.azure.net/secrets/${GRAPH_TENANT_ID_SECRET_NAME})"
  )
fi
if [[ -n "$GRAPH_CLIENT_ID_SECRET_NAME" ]]; then
  GRAPH_APP_SETTINGS+=(
    "FLEETPULSE_GRAPH_CLIENT_ID=@Microsoft.KeyVault(SecretUri=https://${KV_NAME}.vault.azure.net/secrets/${GRAPH_CLIENT_ID_SECRET_NAME})"
  )
fi
if [[ -n "$GRAPH_CLIENT_SECRET_SECRET_NAME" ]]; then
  GRAPH_APP_SETTINGS+=(
    "FLEETPULSE_GRAPH_CLIENT_SECRET=@Microsoft.KeyVault(SecretUri=https://${KV_NAME}.vault.azure.net/secrets/${GRAPH_CLIENT_SECRET_SECRET_NAME})"
  )
fi
...
az webapp config appsettings set \
  ...
  --settings \
    ...
    HR_RECRUITING_SOURCE="$HR_SOURCE" \
    HR_RECRUITING_WORKBOOK_PATH="$HR_WORKBOOK_PATH" \
    HR_RECRUITING_CONVERSION_WORKBOOK_PATH="$HR_CONVERSION_WORKBOOK_PATH" \
    HR_RECRUITING_STATE_PATH="$HR_STATE_PATH" \
    HR_CALL_ANALYSIS_STATE_PATH="$HR_CALL_ANALYSIS_STATE_PATH" \
    DEPARTMENT_CALL_ANALYSIS_STATE_PATH="$DEPARTMENT_CALL_ANALYSIS_STATE_PATH" \
    ...
    HR_CALL_ANALYSIS_SHAREPOINT_FILE_EXTENSIONS="${HR_CALL_ANALYSIS_SHAREPOINT_FILE_EXTENSIONS:-.txt,.csv}" \
    HR_CALL_ANALYSIS_SYNC_INTERVAL_MINUTES="${HR_CALL_ANALYSIS_SYNC_INTERVAL_MINUTES:-15}" \
    HR_CALL_ANALYSIS_ACTIVE_EXTENSIONS="${HR_CALL_ANALYSIS_ACTIVE_EXTENSIONS:-702,722,725,728,700}" \
